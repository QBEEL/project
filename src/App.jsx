      ...
      <section id='lead' className='py-16 lg:py-24 bg-indigo-600 text-white'>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center'>
          ...
          <Card className='rounded-2xl bg-white text-slate-900'>
            ...
            <CardContent>
              <form name='lead' method='POST' data-netlify='true' netlify-honeypot='bot-field' className='space-y-4'>
                <input type='hidden' name='form-name' value='lead' />
                <p className='hidden'>
                  <label>Не заполнять: <input name='bot-field' /></label>
                </p>
                <Input name='parent' placeholder='Имя родителя' required />
                <Input name='child' placeholder='Имя ребёнка' />
                <Input name='phone' type='tel' placeholder='Телефон' required />
                <Input name='email' type='email' placeholder='Email (необязательно)' />
                <Textarea name='details' placeholder='Интересующий курс / возраст / удобное время' rows={4} />
